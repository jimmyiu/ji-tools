...
**TabBar (mobile capsule):** Active state SHALL be indicated purely through icon and text styling — the icon uses `fill="currentColor"` (solid) with `text-primary`, and there is no background highlight. See the `tab-bar/spec.md` for full TabBar active state details.
...
The application SHALL define two CSS custom properties that swap at the 1024px breakpoint: `--nav-bottom-offset` and `--nav-left-offset`. Mobile default: `--nav-bottom-offset: calc(104px + env(safe-area-inset-bottom))` (64px capsule height + 16px spacing + 24px bottom offset + safe area) and `--nav-left-offset: 0px`. Desktop override: `--nav-bottom-offset: 0px` and `--nav-left-offset: 80px`. The `<main>` element SHALL use `paddingBottom: var(--nav-bottom-offset)` and `paddingLeft: var(--nav-left-offset)`. The outer `<header>` element SHALL use `paddingLeft: var(--nav-left-offset)` on desktop (NOT the inner max-w-5xl container, which must preserve its `px-4` padding — applying paddingLeft to the inner container would override `px-4` on mobile where `--nav-left-offset` is `0px`). A `transition-[padding] duration-150 ease-out` SHALL be applied to `<main>` only (NOT the header which has JS-driven scroll animation).
