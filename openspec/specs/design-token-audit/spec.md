...
#### Scenario: Phase bar uses phase-bar token with dynamic opacity
- **WHEN** a phase segment renders
- **THEN** its background color SHALL combine `--color-phase-bar` with its dynamic opacity value using `color-mix()`

#### Scenario: Phase bar label uses phase-bar-foreground
- **WHEN** the phase bar label text renders
- **THEN** it SHALL use `text-phase-bar-foreground` instead of `text-purple-200`

#### Scenario: Phase rate text uses phase-rate-usd token
- **WHEN** a phase rate value renders
- **THEN** it SHALL use `text-phase-rate-usd` instead of `text-green-300`
...
### Requirement: New positive, negative, shadow, and phase-bar CSS variables

The system SHALL define `--positive`, `--positive-foreground`, `--negative`, `--negative-foreground`, `--tab-bar-shadow`, `--phase-bar`, `--phase-bar-foreground`, and `--phase-rate-usd` CSS custom properties in `src/index.css` under the `.dark` block, and map them in the `@theme inline` block for Tailwind utility use.
...
#### Scenario: Phase bar token is usable via Tailwind
- **WHEN** the CSS is processed
- **THEN** `bg-phase-bar`, `text-phase-bar` SHALL resolve via `@theme inline`

#### Scenario: Phase bar foreground token is usable via Tailwind
- **WHEN** the CSS is processed
- **THEN** `text-phase-bar-foreground` SHALL resolve via `@theme inline`

#### Scenario: Phase rate USD token is usable via Tailwind
- **WHEN** the CSS is processed
- **THEN** `text-phase-rate-usd` SHALL resolve via `@theme inline`
