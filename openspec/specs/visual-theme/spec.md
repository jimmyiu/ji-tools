...
Defines the CSS custom property values for shadcn/ui design tokens in the dark-only theme at `src/index.css`. Establishes a systematic elevation hierarchy (background → card → input → border) with a 260° blue-grey slate hue and indigo primary at 270°.
